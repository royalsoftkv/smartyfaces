...
{literal}
	...
	{sf_form}
		...
		<div class="d-flex gap-3 align-items-center">
			Name:
			{sf_inputtext value="#[\$bean->countries->filter['name']]"}
			Numeric code:
			{sf_inputtext value="#[\$bean->countries->filter['numcode']]"}
			{sf_commandbutton value="Search" action="#[\$bean->countries->search()]"}
			{sf_commandbutton btnclass=secondary value="Reset search" action="#[\$bean->countries->resetSearch()]" disabled=$bean->countries->isEmptyFilter()}
		</div>
		<hr/>
...
{sf_tabs id="tabs"}
	...
	{sf_tab header="PHP code - DataModelView"}
		<textarea id="code_php">{DemoFunctions::getClassNameSource('DataModelView')}</textarea>
	{/sf_tab}
	{sf_tab header="PHP code - CountryDataModel"}
		<textarea id="code_php_2">{DemoFunctions::getClassNameSource('CountryDataModel')}</textarea>
	{/sf_tab}
{/sf_tabs}
